...
import {Container, CurrencyCode, MasterType} from "../../_model/CurrencyCode";
...
import {allowedServices, TableColumn, TableName} from "../../_model/TableColumn";
...
export class ListInputTableComponent implements OnInit {
  ...
  Columns:TableColumn<allowedServices>[]
  ...
  ngOnInit(): void {
    ...
    this.$subscription = this.buttonControlService.tableName.subscribe((name)=>{
  ...
  getAllData(){
    ...
    this.parentService.getAllData(this.resultsLength).pipe(pluck('result')).subscribe((data)=>{
  ...
  saveChanges(row: allowedServices) {
    ...
    this.parentService.saveChanges(row).subscribe((res)=>{
  ...
  getListOptions(key,value){
     let filteredResult = this.allTableData.filter((data)=>{
      return data[key] !==null && (data[key].toLowerCase()).includes(value.toLowerCase())
    })
